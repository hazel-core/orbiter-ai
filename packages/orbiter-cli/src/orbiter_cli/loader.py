"""Agent discovery and loading for the Orbiter CLI.
...
2. **YAML files** (``.yaml``) — agent configs loaded via
   :func:`orbiter.loader.load_agents`.
...
def load_python_agent(path: Path) -> dict[str, Any]:
    ...
    """
    module_name = f"_orbiter_agent_{path.stem}"
...
def load_yaml_agents(path: Path) -> dict[str, Any]:
    ...
    Delegates to :func:`orbiter.loader.load_agents`.
    """
    from orbiter.loader import LoaderError, load_agents
