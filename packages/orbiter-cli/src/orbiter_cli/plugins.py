"""Plugin system for extending Orbiter CLI functionality.
...
1. **Entry points** — packages declaring the ``orbiter.plugins`` group
   in their metadata (e.g. ``[project.entry-points."orbiter.plugins"]``).
...
_ENTRYPOINT_GROUP = "orbiter.plugins"
...
class PluginManager:
    ...
    def load_entrypoints(self) -> int:
        """Discover and load plugins from ``orbiter.plugins`` entry points.
...
def _load_plugin_file(path: Path) -> PluginSpec | None:
    ...
    module_name = f"_orbiter_plugin_{path.stem}"
