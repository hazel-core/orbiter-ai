"""Tests for orbiter_cli.plugins — plugin system."""
...
from orbiter_cli.plugins import (
    PluginError,
    PluginHook,
    PluginManager,
    PluginSpec,
...
class TestPluginManagerLoadDirectory:
    def test_load_valid_plugin(self, tmp_path: Path) -> None:
        ...
        plugin_file.write_text(
            textwrap.dedent("""\
                from orbiter_cli.plugins import PluginSpec
...
class TestPluginManagerEntrypoints:
    def test_load_entrypoint(self) -> None:
        ...
        with patch("orbiter_cli.plugins.importlib.metadata.entry_points", return_value=mock_eps):
            mgr = PluginManager()
