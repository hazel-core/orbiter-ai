"""Tests for orbiter_server.app — FastAPI app with /chat endpoint."""
...
from orbiter_server.app import (
    ChatRequest,
    ChatResponse,
    _get_agent,
    create_app,
...
class TestRegisterAgent:
    def test_register_single(self) -> None:
        ...
        agents = getattr(app.state, "orbiter_agents", {})
        assert "alpha" in agents
        # First agent auto-becomes default
        assert getattr(app.state, "orbiter_default_agent", None) == "alpha"
    ...
    def test_register_multiple(self) -> None:
        ...
        assert getattr(app.state, "orbiter_default_agent", None) == "a"

    def test_register_explicit_default(self) -> None:
        app = create_app()
        register_agent(app, _mock_agent("a"))
        register_agent(app, _mock_agent("b"), default=True)
        assert getattr(app.state, "orbiter_default_agent", None) == "b"
...
class TestGetAgent:
    ...
    def test_no_default_no_name(self) -> None:
        ...
        app.state.orbiter_agents = {"a": _mock_agent("a")}  # type: ignore[attr-defined]
        app.state.orbiter_default_agent = None  # type: ignore[attr-defined]
...
class TestChatEndpoint:
    async def test_chat_success(self) -> None:
        ...
        with patch("orbiter_server.app._run_agent", new_callable=AsyncMock) as mock_run:
...
class TestChatStreaming:
    async def test_stream_text_events(self) -> None:
        ...
        with patch("orbiter_server.app._run_agent", mock_run):
            async with _build_client(app) as client:
...
class TestCreateApp:
    def test_creates_fastapi_app(self) -> None:
        app = create_app()
        assert app.title == "Orbiter Server"
