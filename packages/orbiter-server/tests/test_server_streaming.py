"""Tests for orbiter_server.streaming — WebSocket + SSE streaming."""
...
from orbiter_server.app import create_app, register_agent
from orbiter_server.streaming import _iter_events, _resolve_agent
...
class TestResolveAgent:
    ...
    def test_default(self) -> None:
        ...
        state = MagicMock()
        state.orbiter_agents = {"alpha": agent}
        state.orbiter_default_agent = "alpha"
        assert _resolve_agent(state, None) is agent

    def test_no_default(self) -> None:
        state = MagicMock()
        state.orbiter_agents = {"a": _mock_agent("a")}
        state.orbiter_default_agent = None
        assert _resolve_agent(state, None) is None
...
class TestIterEvents:
    async def test_text_events(self) -> None:
        ...
        with patch("orbiter_server.streaming._run_agent", mock_run):
