"""Orbiter Server: FastAPI app with /chat endpoint.

Provides a web API for running Orbiter agents via HTTP.
...
Usage::
    ...
    # Run with: uvicorn orbiter_server.app:app
"""
...
_AGENTS_KEY = "orbiter_agents"
_DEFAULT_AGENT_KEY = "orbiter_default_agent"
...
def register_agent(app: FastAPI, agent: Any, *, default: bool = False) -> None:
    ...
    agents[name] = agent
    app.state.orbiter_agents = agents  # type: ignore[attr-defined]
    if default or len(agents) == 1:
        app.state.orbiter_default_agent = name  # type: ignore[attr-defined]
