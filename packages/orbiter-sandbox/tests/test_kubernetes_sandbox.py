...
class TestInit:
    ...
    def test_env_var_namespace(self, monkeypatch: pytest.MonkeyPatch) -> None:
        monkeypatch.setenv("ORBITER_K8S_NAMESPACE", "staging")
        sb = KubernetesSandbox()
        assert sb.namespace == "staging"

    def test_env_var_image(self, monkeypatch: pytest.MonkeyPatch) -> None:
        monkeypatch.setenv("ORBITER_K8S_IMAGE", "custom:v2")
        sb = KubernetesSandbox()
...
class TestManifests:
    def test_pod_manifest(self) -> None:
        ...
        assert manifest["metadata"]["name"] == "orbiter-abc"
    ...
    def test_service_manifest(self) -> None:
        ...
        assert manifest["metadata"]["name"] == "orbiter-svc-xyz"
...
class TestStart:
    ...
    async def test_start_pod_timeout(self) -> None:
        ...
        with (
            patch("orbiter.sandbox.kubernetes._MAX_POLL_ATTEMPTS", 2),
            patch("orbiter.sandbox.kubernetes._POLL_INTERVAL", 0.01),
...
class TestDescribe:
    ...
    async def test_describe_after_start(self) -> None:
        ...
        info = sb.describe()
        assert info["pod_name"] == "orbiter-d2"
        assert info["service_name"] == "orbiter-svc-d2"
