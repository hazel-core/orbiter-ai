...
async def _test_provider_connection(
    ...
    try:
        if provider_type == "openai":
            ...
            url = (
                f"{base_url}/v1/models" if base_url else "https://api.anthropic.com/v1/models"
            )
            ...
            if not base_url:
                ...
                return {"success": False, "error": "Vertex requires a base_url", "latency_ms": elapsed}
            url = base_url
            headers = {"Authorization": f"Bearer {api_key}"}
            ...
            if not base_url:
                elapsed = (time.monotonic() - start) * 1000
                return {"success": False, "error": "Custom provider requires a base_url", "latency_ms": elapsed}
            url = base_url
