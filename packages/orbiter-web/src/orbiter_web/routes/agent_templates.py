...
async def create_template(
    ...
    async with get_db() as db:
        ...
        await _create_version(db, template_id, clean_config, body.tools_required, body.models_required)
        await db.commit()
...
async def create_template_from_agent(
    ...
    async with get_db() as db:
        ...
        models_required = json.dumps([f"{agent.get('model_provider', '')}:{agent.get('model_name', '')}"])
...
async def delete_template(
    ...
    async with get_db() as db:
        ...
        await db.execute("DELETE FROM agent_template_versions WHERE template_id = ?", (template_id,))
