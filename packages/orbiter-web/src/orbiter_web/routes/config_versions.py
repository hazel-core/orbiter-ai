...
async def create_config_version(
    ...
    await db.execute(
        ...
        VALUES (?, ?, ?, ?, ?, ?, ?, ?)
        """,
        (version_id, entity_type, entity_id, next_version, json.dumps(config), author, summary, now),
    )
    return next_version
...
@router.post("/api/v1/agents/{agent_id}/versions/{version_id}/rollback", response_model=ConfigVersionResponse)
async def rollback_agent_version(
    ...
    async with get_db() as db:
        ...
        now = datetime.now(UTC).strftime("%Y-%m-%d %H:%M:%S")
        fields = [
            "name", "description", "instructions", "model_provider", "model_name",
            "temperature", "max_tokens", "max_steps", "output_type_json",
            "tools_json", "handoffs_json", "hooks_json",
        ]
...
@router.post("/api/v1/workflows/{workflow_id}/versions/{version_id}/rollback", response_model=ConfigVersionResponse)
