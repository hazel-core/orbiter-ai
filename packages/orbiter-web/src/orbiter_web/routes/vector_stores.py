...
_BACKEND_DEFAULTS: dict[str, dict[str, object]] = {
    ...
    "milvus": {"host": "localhost", "port": 19530, "collection_name": "orbiter"},
    "qdrant": {"host": "localhost", "port": 6333, "collection_name": "orbiter"},
    "chromadb": {"host": "localhost", "port": 8000, "collection_name": "orbiter"},
    "pinecone": {"host": "", "port": None, "collection_name": "orbiter"},
}
...
async def test_connection(user: dict = Depends(get_current_user)) -> TestResult:  # noqa: B008
    ...
    if backend in ("milvus", "qdrant", "chromadb") and not host:
        return TestResult(success=False, error="Host is required", backend=backend, backend_label=label)

    if backend == "pinecone" and not api_key_enc:
        return TestResult(success=False, error="API key is required for Pinecone", backend=backend, backend_label=label)
