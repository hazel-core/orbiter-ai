...
async def _send_test_trace(
    ...
    sample_trace = {
        "name": "orbiter-test-trace",
        "input": {"message": "Hello from Orbiter"},
    ...
    async with httpx.AsyncClient(timeout=15.0) as client:
        if platform == "langfuse":
            await _test_langfuse(client, endpoint_url, api_key, project_name, extra_config, sample_trace)
        ...
        "batch": [
            {
                ...
                "body": {
                    ...
                    "input": trace["input"],
                    "output": trace["output"],
                    "metadata": {"source": "orbiter-test", "project": project_name},
                },
            }
        ],
    }
...
async def _test_langsmith(
    ...
    payload = {
        ...
        "session_name": project_name or "orbiter-test",
...
async def _test_datadog(
    ...
    payload = [
        {
            "ddsource": "orbiter",
            "service": "orbiter-agent",
            "message": json.dumps(trace),
            "ddtags": "env:test,source:orbiter",
        }
    ]
...
async def _test_custom_webhook(
    ...
    if api_key:
        ...
        header_value = extra_config.get("auth_prefix", "Bearer") + " " + api_key if extra_config.get("auth_prefix") else api_key
