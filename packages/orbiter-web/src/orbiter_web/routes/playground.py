...
async def _stream_openai(
    ...
    async with httpx.AsyncClient(timeout=120.0) as client, client.stream(
        ...
        if resp.status_code >= 400:
            ...
            await websocket.send_json({"type": "error", "message": f"API error ({resp.status_code}): {error_body.decode()[:300]}"})
            return
...
async def _stream_gemini(
    ...
    async with httpx.AsyncClient(timeout=120.0) as client, client.stream("POST", url, json=body) as resp:
        if resp.status_code >= 400:
            error_body = await resp.aread()
            await websocket.send_json({"type": "error", "message": f"API error ({resp.status_code}): {error_body.decode()[:300]}"})
            return
...
async def playground_chat(websocket: WebSocket, agent_id: str) -> None:
    ...
    if not provider_id:
        await websocket.send_json({"type": "error", "message": f"No {provider_type} provider configured"})
        await websocket.close()
        return

    provider, api_key = await _resolve_api_key(provider_id, user["id"])
    if not api_key:
        await websocket.send_json({"type": "error", "message": "No API key configured for provider"})
        await websocket.close()
        return
    ...
    # Conversation history (per-session, in-memory for this connection)
        ...
        while True:
            ...
            try:
                if provider_type in ("openai", "custom"):
                    await _stream_openai(websocket, api_key, base_url, model_name, history, temperature, max_tokens)
                elif provider_type == "anthropic":
                    await _stream_anthropic(websocket, api_key, model_name, system_prompt, history, temperature, max_tokens)
                elif provider_type == "gemini":
                    await _stream_gemini(websocket, api_key, model_name, history, temperature, max_tokens)
                elif provider_type == "ollama":
                    await _stream_ollama(websocket, base_url, model_name, history, temperature)
                else:
                    await websocket.send_json({"type": "error", "message": f"Unsupported provider: {provider_type}"})
                    continue
            except Exception as exc:
                await websocket.send_json({"type": "error", "message": f"Stream error: {exc!s}"})
                continue

            # Add a placeholder assistant message to history for context
            # (The actual content was streamed — we don't collect it here for simplicity.
            #  Future stories will persist full messages to DB.)
