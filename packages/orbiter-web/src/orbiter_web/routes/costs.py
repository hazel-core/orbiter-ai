...
async def update_pricing(
    ...
    async with get_db() as db:
        cursor = await db.execute(
            "SELECT id FROM model_pricing WHERE id = ?", (pricing_id,)
        )
...
async def get_cost_summary(
    ...
    cost_by_agent = [
        {"name": r["agent_name"], "id": r["agent_id"], "cost": round(r["cost"] or 0, 6), "runs": r["run_count"]}
    ...
    async with get_db() as db:
        ...
        if existing:
            ...
            await db.execute(
                ...
                VALUES (?, ?, ?, ?, ?, ?, ?)""",
                (budget_id, body.scope, body.scope_id, body.budget_amount, body.period, body.alert_threshold, uid),
            )
        await db.commit()
