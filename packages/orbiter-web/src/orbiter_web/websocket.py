"""Multiplexed WebSocket manager for Orbiter Web.
...
VALID_CHANNELS = frozenset(
    {"chat", "execution", "logs", "sandbox", "notifications", "system"}
)
...
class WebSocketManager:
    ...
    async def send_to_connection(
        ...
        return await conn.send(
            {"channel": channel, "type": msg_type, "payload": payload}
        )

    async def broadcast_to_user(
        self, user_id: str, channel: str, message: dict[str, Any]
    ) -> int:
...
async def _handle_message(ws: WebSocket, raw: str) -> None:
    ...
    try:
        ...
        await manager.send_to_connection(
            ws, "system", "error", {"message": "Invalid JSON"}
        )
        return
    ...
    if msg_type == "subscribe":
        ...
        await manager.send_to_connection(
            ws, "system", "subscribed", {"channel": channel}
        )
        ...
        await manager.send_to_connection(
            ws, "system", "unsubscribed", {"channel": channel}
        )
...
async def websocket_endpoint(websocket: WebSocket) -> None:
    ...
    Authentication: the browser sends the ``orbiter_session`` cookie
