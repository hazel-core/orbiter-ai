...
async def complete_step(
    ...
    async with get_db() as db:
        ...
        cursor = await db.execute(
            "SELECT plan_id FROM agent_plan_steps WHERE id = ?", (step_id,)
        )
        row = await cursor.fetchone()
        if row:
...
def build_executor_prompt(step_description: str, agent_instructions: str, prior_outputs: list[dict[str, Any]]) -> str:
