"""Bridge between database agent configs and live Orbiter Agent objects.

Converts stored agent configurations (model, tools, instructions, etc.) into
runnable ``orbiter.Agent`` instances backed by real ``ModelProvider`` objects.
"""
...
async def _resolve_provider(
    provider_type: str, model_name: str, user_id: str
) -> ModelProvider:
    ...
    async with get_db() as db:
        ...
        if provider_row is None:
            raise AgentRuntimeError(
                f"No {provider_type} provider configured for user {user_id}"
            )
    ...
    if not api_key:
        raise AgentRuntimeError(
            f"No API key configured for {provider_type} provider"
        )
...
async def _resolve_tools(
    tools_json: str, project_id: str, user_id: str
) -> list[FunctionTool]:
...
class AgentService:
    """Converts database agent config records into live Orbiter Agent objects."""
    ...
    async def build_agent(self, agent_id: str) -> Agent:
        ...
        Returns:
            A configured ``orbiter.Agent`` ready for execution.
        ...
        if not provider_type or not model_name:
            raise AgentRuntimeError(
                f"Agent {agent_id} has no model configured"
            )
