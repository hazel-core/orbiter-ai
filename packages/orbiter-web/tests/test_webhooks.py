...
from orbiter_web.routes.webhooks import router
...
class TestWebhookTriggerRequiresToken:
    ...
    def test_missing_token_returns_403(self) -> None:
        ...
        with patch("orbiter_web.routes.webhooks.get_db", mock_get_db):
    ...
    def test_correct_token_returns_200(self) -> None:
        ...
        with (
            patch("orbiter_web.routes.webhooks.get_db", mock_get_db),
            patch("orbiter_web.services.run_queue.can_start_run", AsyncMock(return_value=True)),
            patch("orbiter_web.engine.execute_workflow", AsyncMock()),
