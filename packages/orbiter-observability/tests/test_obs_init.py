"""Tests for orbiter.observability public API (__init__.py exports)."""
...
class TestEagerImports:
    """Core symbols are importable directly from orbiter.observability."""

    def test_configure(self) -> None:
        from orbiter.observability import configure  # pyright: ignore[reportMissingImports]

        assert callable(configure)

    def test_get_logger(self) -> None:
        from orbiter.observability import get_logger  # pyright: ignore[reportMissingImports]

        assert callable(get_logger)

    def test_traced(self) -> None:
        from orbiter.observability import traced  # pyright: ignore[reportMissingImports]

        assert callable(traced)

    def test_span(self) -> None:
        from orbiter.observability import span  # pyright: ignore[reportMissingImports]

        assert callable(span)

    def test_aspan(self) -> None:
        from orbiter.observability import aspan  # pyright: ignore[reportMissingImports]

        assert callable(aspan)

    def test_get_config(self) -> None:
        from orbiter.observability import get_config  # pyright: ignore[reportMissingImports]

        assert callable(get_config)

    def test_configure_logging(self) -> None:
        from orbiter.observability import configure_logging  # pyright: ignore[reportMissingImports]

        assert callable(configure_logging)

    def test_log_context(self) -> None:
        from orbiter.observability import LogContext  # pyright: ignore[reportMissingImports]

        assert LogContext is not None

    def test_observability_config(self) -> None:
        from orbiter.observability import (  # pyright: ignore[reportMissingImports]
            ObservabilityConfig,
        )
    ...
    def test_trace_backend(self) -> None:
        from orbiter.observability import TraceBackend  # pyright: ignore[reportMissingImports]
...
class TestLazyImports:
    """Lazy-loaded symbols are accessible via orbiter.observability.<name>."""

    def test_metrics_collector(self) -> None:
        from orbiter.observability import MetricsCollector  # pyright: ignore[reportMissingImports]

        assert MetricsCollector is not None

    def test_timer(self) -> None:
        from orbiter.observability import Timer  # pyright: ignore[reportMissingImports]

        assert Timer is not None

    def test_null_span(self) -> None:
        from orbiter.observability import NullSpan  # pyright: ignore[reportMissingImports]

        assert NullSpan is not None

    def test_health_status(self) -> None:
        from orbiter.observability import HealthStatus  # pyright: ignore[reportMissingImports]

        assert HealthStatus is not None

    def test_alert_severity(self) -> None:
        from orbiter.observability import AlertSeverity  # pyright: ignore[reportMissingImports]

        assert AlertSeverity is not None

    def test_cost_tracker(self) -> None:
        from orbiter.observability import CostTracker  # pyright: ignore[reportMissingImports]

        assert CostTracker is not None

    def test_slo_tracker(self) -> None:
        from orbiter.observability import SLOTracker  # pyright: ignore[reportMissingImports]

        assert SLOTracker is not None

    def test_prompt_logger(self) -> None:
        from orbiter.observability import PromptLogger  # pyright: ignore[reportMissingImports]

        assert PromptLogger is not None

    def test_baggage_propagator(self) -> None:
        from orbiter.observability import BaggagePropagator  # pyright: ignore[reportMissingImports]

        assert BaggagePropagator is not None

    def test_semconv_constant(self) -> None:
        from orbiter.observability import AGENT_NAME  # pyright: ignore[reportMissingImports]

        assert AGENT_NAME == "orbiter.agent.name"

    def test_record_agent_run(self) -> None:
        from orbiter.observability import record_agent_run  # pyright: ignore[reportMissingImports]

        assert callable(record_agent_run)

    def test_health_registry(self) -> None:
        from orbiter.observability import HealthRegistry  # pyright: ignore[reportMissingImports]

        assert HealthRegistry is not None

    def test_alert_manager(self) -> None:
        from orbiter.observability import AlertManager  # pyright: ignore[reportMissingImports]

        assert AlertManager is not None

    def test_model_pricing(self) -> None:
        from orbiter.observability import ModelPricing  # pyright: ignore[reportMissingImports]
...
class TestSubModuleImports:
    ...
    def test_from_tracing(self) -> None:
        from orbiter.observability.tracing import traced  # pyright: ignore[reportMissingImports]
    ...
    def test_from_alerts(self) -> None:
        from orbiter.observability.alerts import (  # pyright: ignore[reportMissingImports]
            AlertManager,
        )
    ...
    def test_from_cost(self) -> None:
        from orbiter.observability.cost import CostTracker  # pyright: ignore[reportMissingImports]

        assert CostTracker is not None

    def test_from_slo(self) -> None:
        from orbiter.observability.slo import SLOTracker  # pyright: ignore[reportMissingImports]
...
class TestAllCompleteness:
    ...
    def test_all_is_list(self) -> None:
        import orbiter.observability as obs  # pyright: ignore[reportMissingImports]
...
class TestOTelOptional:
    ...
    def test_import_without_otel(self) -> None:
        ...
        # no OTel.
        with (
            patch("orbiter.observability.tracing.HAS_OTEL", False),
            patch("orbiter.observability.metrics.HAS_OTEL", False),
        ):
            # Re-import to verify no errors
            importlib.reload(sys.modules["orbiter.observability"])
            import orbiter.observability as obs  # pyright: ignore[reportMissingImports]
