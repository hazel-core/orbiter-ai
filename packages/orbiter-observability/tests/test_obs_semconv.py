"""Tests for orbiter.observability.semconv — semantic conventions."""
...
from orbiter.observability import semconv  # pyright: ignore[reportMissingImports]
    ...
    def test_agent_constants_prefixed(self) -> None:
        for name, value in _public_constants().items():
            if name.startswith("AGENT_"):
                assert value.startswith("orbiter.agent."), (
                    f"{name}={value!r} should start with 'orbiter.agent.'"
                )

    def test_tool_constants_prefixed(self) -> None:
        for name, value in _public_constants().items():
            if name.startswith("TOOL_"):
                assert value.startswith("orbiter.tool."), (
                    f"{name}={value!r} should start with 'orbiter.tool.'"
                )

    def test_cost_constants_prefixed(self) -> None:
        for name, value in _public_constants().items():
            if name.startswith("COST_"):
                assert value.startswith("orbiter.cost."), (
                    f"{name}={value!r} should start with 'orbiter.cost.'"
                )
...
# ---------------------------------------------------------------------------
# Specific value checks (parity with orbiter-trace config.py)
# ---------------------------------------------------------------------------
...
class TestExactValues:
    ...
    def test_agent_id(self) -> None:
        assert semconv.AGENT_ID == "orbiter.agent.id"

    def test_agent_name(self) -> None:
        assert semconv.AGENT_NAME == "orbiter.agent.name"

    def test_tool_name(self) -> None:
        assert semconv.TOOL_NAME == "orbiter.tool.name"

    def test_session_id(self) -> None:
        assert semconv.SESSION_ID == "orbiter.session.id"
    ...
    def test_cost_total_usd(self) -> None:
        assert semconv.COST_TOTAL_USD == "orbiter.cost.total_usd"
