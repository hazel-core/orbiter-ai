"""Structured logging for Orbiter — stdlib only.
...
Usage::
    ...
    from orbiter.observability.logging import get_logger, configure_logging, LogContext

    log = get_logger("agent")          # -> orbiter.agent
...
class TextFormatter(logging.Formatter):
    """Compact single-line ANSI formatter (ported from orbiter.log)."""

    def format(self, record: logging.LogRecord) -> str:
        ts = self.formatTime(record, "%H:%M:%S")
        lvl = _LEVEL_CHAR.get(record.levelno, "?")
        color = _COLORS.get(record.levelno, "")
        # Strip "orbiter." prefix for shorter output
        name = record.name
...
def get_logger(name: str) -> logging.Logger:
    """Return a stdlib ``Logger`` under the ``orbiter.`` namespace.

    If *name* does not start with ``orbiter.``, it is auto-prefixed.
    """
...
def configure_logging(
    ...
    *,
    ...
    """One-time handler setup on the ``orbiter`` root logger.
