...
class TestOffloadLargeResult:
    ...
    async def test_offload_emits_debug_log(self, caplog: pytest.LogCaptureFixture) -> None:
        ...
        agent = Agent(name="bot", memory=None, context=None)
        content = "hello world"
        with caplog.at_level(logging.DEBUG, logger="orbiter.agent"):
    ...
    async def test_offload_fallback_when_context_not_installed(self) -> None:
        """When orbiter-context is not importable, returns content unchanged."""
        import sys

        agent = Agent(name="bot", memory=None, context=None)
        original = sys.modules.get("orbiter.context.workspace")
        sys.modules["orbiter.context.workspace"] = None  # type: ignore[assignment]
        try:
            content = "some data"
            ...
            if original is None:
                sys.modules.pop("orbiter.context.workspace", None)
            else:
                sys.modules["orbiter.context.workspace"] = original
...
class TestToolResultOffloaderLargeOutputFlag:
    ...
    async def test_offloader_fires_on_large_output_flag_regardless_of_size(self) -> None:
        ...
        from orbiter.context.processor import ToolResultOffloader  # pyright: ignore[reportMissingImports]
        ...
        assert "offloaded" in payload["tool_result"].lower() or "Result stored" in payload["tool_result"] or "truncated" in payload["tool_result"].lower()
    ...
    @pytest.mark.asyncio
    async def test_offloader_logs_with_artifact_id(
        ...
        with caplog.at_level(logging.DEBUG, logger="orbiter.context.processor"):
            await offloader.process(ctx, payload)
