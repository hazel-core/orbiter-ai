...
class TestRemoveTool:
    ...
    def test_remove_then_readd(self) -> None:
        ...
        async def _readd() -> None:
            await agent.add_tool(double)

        asyncio.get_event_loop().run_until_complete(_readd())
        assert "double" in agent.tools
...
class TestAddMcpServer:
    @pytest.mark.asyncio
    async def test_add_mcp_server_raises_without_orbiter_mcp(self) -> None:
        """add_mcp_server raises AgentError when orbiter-mcp is not available."""
        agent = Agent(name="bot", memory=None, context=None)
        ...
        with (
            patch.dict(
                ...
                {
                    "orbiter.mcp": None,
                    "orbiter.mcp.client": None,
                    "orbiter.mcp.tools": None,
                },
            ),
        ):
            with pytest.raises(AgentError, match="orbiter-mcp is required"):
    ...
    async def test_add_mcp_server_loads_and_registers_tools(self) -> None:
        ...
        with (
            patch(
                "orbiter.agent.Agent.add_mcp_server.__func__"
                if False
                else "orbiter.mcp.client.MCPServerConnection",
                mock_mcp_conn_cls,
                create=True,
            ),
        ):
            ...
            with patch.dict("sys.modules", {}):
                ...
                # Create a fake orbiter.mcp.client module
                ...
                original_client = sys.modules.get("orbiter.mcp.client")
                original_tools = sys.modules.get("orbiter.mcp.tools")
                sys.modules["orbiter.mcp.client"] = fake_client_mod
                sys.modules["orbiter.mcp.tools"] = fake_tools_mod

                try:
                    await agent.add_mcp_server(fake_config)
                finally:
                    if original_client is None:
                        sys.modules.pop("orbiter.mcp.client", None)
                    else:
                        sys.modules["orbiter.mcp.client"] = original_client
                    if original_tools is None:
                        sys.modules.pop("orbiter.mcp.tools", None)
                    else:
                        sys.modules["orbiter.mcp.tools"] = original_tools
            ...
            *,
            ...
            if call_count == 1:
                ...
                assert tools is None or all(
                    t.get("function", {}).get("name") != "late_tool"
                    for t in (tools or [])
                )
