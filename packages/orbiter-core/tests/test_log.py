"""Tests for orbiter.log — minimal pretty logging."""
...
from orbiter.observability.logging import reset_logging  # pyright: ignore[reportMissingImports]


class TestGetLogger:
    def test_returns_child_of_orbiter(self) -> None:
        log = get_logger("mymodule")
        assert log.name == "orbiter.mymodule"
        assert isinstance(log, logging.Logger)

    def test_already_prefixed(self) -> None:
        log = get_logger("orbiter.agent")
...
class TestFormatter:
    ...
    def test_strips_orbiter_prefix(self) -> None:
        fmt = _Formatter()
        record = self._make_record(name="orbiter.runner", msg="test")
        output = fmt.format(record)
        assert "runner" in output
        # Should not contain "orbiter.runner" as the full dotted name
