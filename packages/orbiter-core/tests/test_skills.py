"""Tests for orbiter.skills — multi-source skill registry."""
...
class TestSkillError:
    def test_inherits_orbiter_error(self) -> None:
        assert issubclass(SkillError, OrbiterError)
...
class TestSkillRegistryGitHub:
    def test_github_source_clones(self, tmp_path: Path) -> None:
        ...
        with patch("orbiter.skills._clone_github", return_value=clone_dir):
            skills = reg.load_all()
    ...
    def test_github_with_subdir(self, tmp_path: Path) -> None:
        ...
        with patch("orbiter.skills._clone_github", return_value=sub):
            skills = reg.load_all()
