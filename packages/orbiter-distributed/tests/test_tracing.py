...
from orbiter.distributed.client import distributed  # pyright: ignore[reportMissingImports]
...
class TestDistributedSubmissionSpan:
    ...
    async def test_creates_submit_span(self) -> None:
        """distributed() should create an 'orbiter.distributed.submit' span."""
        agent = MagicMock()
        ...
        with (
            patch(
                "orbiter.distributed.client.TaskBroker"
            ) as mock_broker_cls,
            patch("orbiter.distributed.client.TaskStore") as mock_store_cls,
            patch(
                "orbiter.distributed.client.EventSubscriber"
            ) as mock_sub_cls,
            patch("orbiter.distributed.client.aspan") as mock_aspan,
        ):
...
class TestWorkerExecutionSpan:
    ...
    async def test_creates_execute_span(self) -> None:
        """Worker should create an 'orbiter.distributed.execute' span."""
        w = Worker("redis://localhost", worker_id="w1")
        w._broker = AsyncMock()
        w._store = AsyncMock()
        w._publisher = AsyncMock()
        ...
        with (
            ...
            patch.object(w, "_run_agent", side_effect=_fake_run),
            patch.object(w, "_listen_for_cancel", new_callable=AsyncMock),
            patch("orbiter.distributed.worker.aspan") as mock_aspan,
        ):
            mock_span = MagicMock()
            mock_aspan.return_value.__aenter__ = AsyncMock(return_value=mock_span)
            mock_aspan.return_value.__aexit__ = AsyncMock(return_value=False)
    ...
    async def test_execute_span_records_exception_on_failure(self) -> None:
        ...
        with (
            patch.object(
                w, "_reconstruct_agent", side_effect=ValueError("bad config")
            ),
            patch.object(w, "_listen_for_cancel", new_callable=AsyncMock),
            patch("orbiter.distributed.worker.aspan") as mock_aspan,
        ):
