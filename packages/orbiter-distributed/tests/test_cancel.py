...
class TestTaskBrokerCancel:
    ...
    async def test_cancel_publishes_signal(self) -> None:
        ...
        with patch("orbiter.distributed.broker.aioredis.from_url") as mock_from_url:
            ...
            mock_redis.publish.assert_called_once_with(
                "orbiter:cancel:task-123", "cancel"
            )
    ...
    async def test_cancel_with_fakeredis(self) -> None:
        ...
        data = await broker._redis.hgetall("orbiter:task:task-789")  # type: ignore[misc]
...
class TestWorkerListenForCancel:
    ...
    async def test_cancel_signal_sets_token(self) -> None:
        ...
        with patch(
            "orbiter.distributed.worker.aioredis.from_url", return_value=fake_redis
        ):
            # Start listener in background
            listener_task = asyncio.create_task(
                w._listen_for_cancel("task-abc", token)
            )
            ...
            await fake_redis.publish("orbiter:cancel:task-abc", "cancel")  # type: ignore[misc]
    ...
    async def test_listener_cancellable(self) -> None:
        ...
        with patch(
            "orbiter.distributed.worker.aioredis.from_url", return_value=mock_redis
        ):
            task = asyncio.create_task(
                w._listen_for_cancel("task-def", token)
            )
            await asyncio.sleep(0.05)
            task.cancel()
...
class TestWorkerRunAgentCancellation:
    ...
    async def test_stops_on_cancellation(self) -> None:
        ...
        published_events = [
            call.args[1] for call in w._publisher.publish.call_args_list
        ]

        # Text events published before cancellation
        text_events = [
            ev for ev in published_events if hasattr(ev, "text")
        ]
        ...
        from orbiter.types import StatusEvent  # pyright: ignore[reportMissingImports]
