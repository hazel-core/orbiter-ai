...
class TestEventPublisherConnect:
    ...
    async def test_connect_creates_client(self) -> None:
        pub = EventPublisher("redis://localhost:6379")
        with patch("orbiter.distributed.events.aioredis.from_url") as mock_from_url:
...
class TestEventPublisherWithFakeRedis:
    ...
    async def test_publish_adds_to_stream(
        ...
        entries = await pub._redis.xrange("orbiter:stream:task-1")  # type: ignore[union-attr]
    ...
    @pytest.mark.asyncio
    async def test_publish_different_tasks(
        self, connected_publisher: EventPublisher
    ) -> None:
        pub = connected_publisher
        ...
        entries_a = await pub._redis.xrange("orbiter:stream:task-A")  # type: ignore[union-attr]
        entries_b = await pub._redis.xrange("orbiter:stream:task-B")  # type: ignore[union-attr]
    ...
    @pytest.mark.asyncio
    async def test_publish_complex_event(
        self, connected_publisher: EventPublisher
    ) -> None:
        pub = connected_publisher
    ...
    @pytest.mark.asyncio
    async def test_publish_sets_stream_ttl(
        self, connected_publisher: EventPublisher
    ) -> None:
        pub = connected_publisher
        await pub.publish("task-1", TextEvent(text="hello"))

        ttl = await pub._redis.ttl("orbiter:stream:task-1")  # type: ignore[union-attr]
...
class TestEventSubscriberReplay:
    ...
    async def test_replay_returns_all_events(
        ...
        await pub.publish(
            "task-1", StatusEvent(status="completed", message="done")
        )
    ...
    async def test_replay_usage_event_nested_model(
        ...
        event = UsageEvent(
            usage=usage, agent_name="a", step_number=1, model="gpt-4"
        )
        await pub.publish("task-1", event)
...
class TestEventSubscriberSubscribe:
    ...
    async def test_subscribe_receives_published_events(
        self, redis_client  # type: ignore[no-untyped-def]
    ) -> None:
