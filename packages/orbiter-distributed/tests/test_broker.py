...
class TestTaskBrokerInit:
    def test_defaults(self, broker: TaskBroker) -> None:
        assert broker._queue_name == "orbiter:tasks"
        assert broker._max_retries == 3
        assert broker._group_name == "orbiter:tasks:group"
    ...
    async def test_connect_creates_consumer_group(self) -> None:
        ...
        with patch("orbiter.distributed.broker.aioredis.from_url") as mock_from_url:
            ...
            mock_redis.xgroup_create.assert_called_once_with(
                "orbiter:tasks", "orbiter:tasks:group", id="0", mkstream=True
            )
...
class TestTaskBrokerWithFakeRedis:
    ...
    @pytest.mark.asyncio
    async def test_claim_returns_none_on_empty(
    ...
    @pytest.mark.asyncio
    async def test_submit_multiple_and_claim_all(
        self, connected_broker: TaskBroker
    ) -> None:
        broker = connected_broker
    ...
    @pytest.mark.asyncio
    async def test_ack_unknown_task_id_is_noop(
        self, connected_broker: TaskBroker
    ) -> None:
        broker = connected_broker
    ...
    @pytest.mark.asyncio
    async def test_nack_unknown_task_id_is_noop(
        self, connected_broker: TaskBroker
    ) -> None:
        broker = connected_broker
