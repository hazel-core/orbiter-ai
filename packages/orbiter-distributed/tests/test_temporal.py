...
class TestTemporalExecutorInit:
    def test_defaults(self) -> None:
        ...
        assert executor.task_queue == "orbiter-tasks"
    ...
    def test_raises_without_temporalio(self) -> None:
        with (
            patch("orbiter.distributed.temporal.HAS_TEMPORAL", False),
...
class TestTemporalExecutorLifecycle:
    ...
    async def test_connect(self) -> None:
        executor = TemporalExecutor()
        with patch("orbiter.distributed.temporal.TemporalClient") as mock_client_cls:
...
class TestTemporalExecutorExecuteTask:
    ...
    async def test_not_connected_raises(self) -> None:
        ...
        with pytest.raises(RuntimeError, match="not connected"):
            await executor.execute_task(
                task, AsyncMock(), AsyncMock(), token, "w1"
            )
    ...
    async def test_submits_workflow_and_returns_output(self) -> None:
        ...
        assert call_kwargs.kwargs["id"] == "orbiter-task-t1"
        assert call_kwargs.kwargs["task_queue"] == "orbiter-tasks"
...
class TestExecuteAgentActivity:
    ...
    async def test_executes_agent_and_returns_output(self) -> None:
        from orbiter.distributed.temporal import (
        ...
        with (
            patch("orbiter.agent.Agent") as mock_agent_cls,
            patch("orbiter.runner.run", mock_run),
    ...
    async def test_detects_swarm_config(self) -> None:
        ...
        with (
            patch("orbiter.swarm.Swarm") as mock_swarm_cls,
            patch("orbiter.runner.run", mock_run),
            patch("temporalio.activity.heartbeat"),
        ):
...
class TestWorkerExecutorParam:
    ...
    def test_temporal_executor_created(self) -> None:
        with patch("orbiter.distributed.temporal.TemporalClient"):
    ...
    def test_temporal_executor_raises_without_lib(self) -> None:
        with (
            patch("orbiter.distributed.worker.HAS_TEMPORAL", False),
