...
_NO_OTEL_EVENTS = patch("orbiter.distributed.events.HAS_OTEL", False)
_NO_OTEL_METRICS = patch("orbiter.distributed.metrics.HAS_OTEL", False)
...
class TestEventPublisherMetrics:
    ...
    @pytest.mark.asyncio
    async def test_publish_records_event_count(
    ...
    @pytest.mark.asyncio
    async def test_publish_records_duration(
        self, connected_publisher: EventPublisher
    ) -> None:
        pub = connected_publisher
        await pub.publish("task-1", TextEvent(text="hello"))
    ...
    @pytest.mark.asyncio
    async def test_publish_multiple_types(
        self, connected_publisher: EventPublisher
    ) -> None:
        pub = connected_publisher
        await pub.publish("task-1", TextEvent(text="hello"))
        await pub.publish(
            "task-1", ToolCallEvent(tool_name="search", tool_call_id="tc-1")
        )
        await pub.publish(
