...
class TestMCPServerConnectionConnect:
    async def test_connect_success(self) -> None:
        ...
        with (
            ...
            patch("orbiter.mcp.client.ClientSession", return_value=session),
        ):
            await conn.connect()
...
class TestMCPServerConnectionTransports:
    def test_create_streams_stdio(self) -> None:
        cfg = _make_stdio_config()
        conn = MCPServerConnection(cfg)
        with patch("orbiter.mcp.client.stdio_client") as mock_stdio:
    ...
    def test_create_streams_sse(self) -> None:
        cfg = _make_sse_config()
        conn = MCPServerConnection(cfg)
        with patch("orbiter.mcp.client.sse_client") as mock_sse:
    ...
    def test_create_streams_streamable_http(self) -> None:
        cfg = _make_http_config()
        conn = MCPServerConnection(cfg)
        with patch("orbiter.mcp.client.streamablehttp_client") as mock_http:
...
class TestMCPClientConnect:
    async def test_connect_single(self) -> None:
        ...
        with (
            patch(
                "orbiter.mcp.client.MCPServerConnection._create_streams",
